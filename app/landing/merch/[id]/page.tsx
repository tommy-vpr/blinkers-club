...
export async function generateStaticParams() {
  return merchData.map((item) => ({ id: item.id.toString() }));
}
...
export default async function Page({
  ...
  const product = merchData.find((item) => item.id.toString() === id);
  if (!product) return notFound();

  const recommendations = merchData
    .filter((item) => item.id.toString() !== id)
    .slice(0, 4);

  return (
    <div className="w-full min-h-screen justify-center items-center bg-white mt-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {recommendations.map((item) => (
              <Link
                ...
                className="cursor-pointer p-3 rounded-xl border transition duration-200 border-transparent hover:border-gray-300"
              >
                <Image
                  src={item.src}
                  alt={item.title}
